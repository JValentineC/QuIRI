...
function ResearchPage() {
  ...
  // Calculate filtered papers for pagination
  const filteredPapers =
    papers?.filter((paper) => {
      const matchesStatus =
        selectedStatus === "all" ||
        paper.status === selectedStatus.toUpperCase();
      const matchesSearch =
        searchTerm === "" ||
  ...
  const {
    currentPage,
    totalPages,
    paginatedData: paginatedPapers,
    goToPage,
  } = usePagination({
    data: filteredPapers,
    itemsPerPage: 6, // Show 6 papers per page
  });
  ...
  const statusCounts = {
    ...
    under_review:
      papers?.filter((p) => p.status === "UNDER_REVIEW").length || 0,
      ...
      <div className="container mx-auto px-4 py-8">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="badge badge-outline">{paper.field}</div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="avatar placeholder mr-3">
                    <div className="bg-neutral text-neutral-content rounded-full w-10">
                      <span className="text-sm">
                        {paper.author.firstName[0]}
                        {paper.author.lastName[0]}
                      </span>
                    </div>
                  </div>
                  <div>
        ...
        {filteredPapers.length > 0 && (
          <div className="text-center text-sm text-base-content/70 mb-4">
            ...
            {Math.min(currentPage * 6, filteredPapers.length)} of{" "}
          ...
          <div className="card bg-gradient-to-r from-primary/10 to-secondary/10 border border-primary/20">
            <div className="card-body">
              ...
              <p className="text-center text-base-content/80 mb-6">
                Contribute to the quantum science community by sharing your
                latest research, findings, and discoveries with peers around the
                world.
              </p>
